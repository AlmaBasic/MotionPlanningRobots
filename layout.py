...
from find_path_iterative import *
...
class Layout:
    def __init__(self, layers, grid, start_coords, end_coords, switch, asset_dict):
        ...
        #self.calc_path(layers, grid, start_coords, end_coords)
        #self.calc_path_iterative(layers, grid, start_coords, end_coords)
    ...
    def calc_path_simple_one_to_one(self, layers, grid, start_coords, end_coords):
        ...
        finder = AStarFinder()
        path = finder.find_path(start_point, end_point, grid) 
        
        self.player.set_path(path)
        
############################################################################  
    def calc_path(self, layers, grid, start_coords, end_coords):
        path =  find_path2(grid, start_coords[0], end_coords[0])
        
        print(path)
        
        #self.player.set_path(path)
        
    def calc_path_iterative(self, layers, grid, start_coords, end_coords):
        path =  shortest_path(50000, grid, start_coords[0], end_coords[0])
